...
json::jtype::jtype json::jtype::detect(const char *input) {
  ...
  case 't':
  case 'f':
    return (strncmp(start, "true", 4) == 0 || strncmp(start, "false", 5) == 0)
               ? json::jtype::jbool
               : json::jtype::not_valid;
    break;
  case 'n':
    return (strncmp(start, "null", 4) == 0) ? json::jtype::jnull
                                            : json::jtype::not_valid;
    break;
  default:
    return json::jtype::not_valid;
...
std::string json::parsing::unescape_characters(const char *input) {
  ...
  if (*input != '\"')
    throw json::parsing_error("Expecting opening quotation");
  input++;
  ...
  while (*input != '"') {
    if (*input != '\\') { // Character is not escaped
      ...
      if (EMPTY_STRING(input))
        throw json::parsing_error("Expecting character");
  ...
  if (*input != '"')
    throw json::parsing_error("Expecting closing quotation");
  return result;
}
...
json::parsing::parse_results json::parsing::parse(const char *input) {
  ...
  case json::jtype::jstring:
    ...
    // Copy the string
    while (!EMPTY_STRING(index)) {
      if (*index != '"' || (result.value.size() > 0 &&
                            result.value[result.value.size() - 1] == '\\')) {
  ...
  case json::jtype::jobject: {
    ...
    while (!EMPTY_STRING(index) && *index != '}') {
      ...
      if (subvalue.type == json::jtype::jstring)
        result.value +=
            json::parsing::escape_characters(subvalue.value.c_str());
      else
  ...
  case json::jtype::jarray: {
    ...
    while (!EMPTY_STRING(index) && *index != ']') {
      ...
      if (array_value.type == json::jtype::jstring)
        result.value +=
            json::parsing::escape_characters(array_value.value.c_str());
      else
...
void json::jobject::proxy::set_array(const std::vector<std::string> &values,
  ...
  for (size_t i = 0; i < values.size(); i++) {
    if (wrap)
      value += json::parsing::escape_characters(values[i].c_str()) + ",";
    else
      value += values[i] + ",";
  }
  if (values.size() > 0)
...
json::jobject::operator std::string() const {
  if (is_array()) {
    ...
    for (size_t i = 0; i < this->size(); i++) {
      result +=
          "\"" + this->data.at(i).first + "\":" + this->data.at(i).second + ",";
    }
    result.erase(result.size() - 1, 1);
    result += "}";
    return result;
